...
export function shouldSkipControlUiPairing(
  ...
  // dangerouslyDisableDeviceAuth is the break-glass path. When enabled, skip
  // pairing for all gateway roles to support cloud/headless deployments.
...
export function evaluateMissingDeviceIdentity(params: {
  ...
  if (params.controlUiAuthPolicy.allowBypass) {
    // dangerouslyDisableDeviceAuth: true — the deployment has explicitly opted
    // out of device-identity enforcement for all gateway websocket clients.
    return { kind: "allow" };
  }
